...
class NoveltyTracer:
    ...
    def build_evidence_graph(
        ...
        for c in interaction_cands:
            ...
            loc_a = c.candidate_name.split("Ã—")[0].strip() if "Ã—" in c.candidate_name else "L10"
            loc_b = c.candidate_name.split("Ã—")[1].split()[0].strip() if "Ã—" in c.candidate_name else "L31"
            G.add_node(
                inter_id,
                label=f"{c.candidate_name} (Î”={round(c.absolute_effect, 1)})",
                type="interaction",
